...
const (
	...
	MARKDOWN_NEW_LINE      = "\n\n"
	URL_ALIYUN_K8S_CONSULE = "https://rancher.k8s.moonton.net/" //mutong rancher
...
func NewMarkdownMsgBuilder(clusterID, region string, event *v1.Event,msgs ...string) *MarkdownMsgBuilder {
	...
	case "Deployment":
		...
		name = fmt.Sprintf(MARKDOWN_LINK_TEMPLATE, event.InvolvedObject.Name, podsURL)
		break
	case "Pod":
	...
	case "CronJob":
		...
		name = fmt.Sprintf(MARKDOWN_LINK_TEMPLATE, event.InvolvedObject.Name, jobURL)
		break
	case "Service":
		...
		name = fmt.Sprintf(MARKDOWN_LINK_TEMPLATE, event.InvolvedObject.Name, svcURL)
		break
